...
echo "*/10 * * * * mongodump --db $MONGO_DB_NAME -u $MONGO_DB_USER -p $MONGO_DB_PASSWORD --authenticationDatabase $MONGO_DB_NAME --out $MONGO_DB_BACKUP_DIRECTORY/\$(date +\%s)/ && /snap/bin/gcloud storage cp $MONGO_DB_BACKUP_DIRECTORY/* gs://mongodb-backups-s3-mpp-not-taken-heliotrope --recursive" | sudo crontab -
